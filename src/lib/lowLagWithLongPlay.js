...
//requests to loadsound, and then call sm2 once it has told us it is set.
	...
	this.loadSoundSM2ForReals = function(urls,ptag){
		...
		urls = lowLag.getURLArray(urls); //coerce
...
lowLag.msg('audioTag loading '+urls+' as tag ' + tag);
		...
		lowLag.audioTagNameToElement[tag] = audioElem;
	...
	this.playSoundAudioTag = function(tag){
		...
		var modelElem = lowLag.audioTagNameToElement[tag];
	...
	this.loadLongPlay = this.loadSoundAudioTag;
	this.playLongPlay = function(tag, loop){
		lowLag.msg("playLongPlay "+tag);
		var modelElem = lowLag.audioTagNameToElement[tag];
		if(!modelElem) return;
		if(! loop) loop = false;
		...
		lowLag.msg("volume "+modelElem.volume);
		lowLag.msg(modelElem.play);
	};
	
	this.loopLongPlay = function(tag){
		this.playLongPlay(tag,true);
	};
	
	this.stopLongPlay = function(tag){
		lowLag.msg("stopLongPlay "+tag);
		var modelElem = lowLag.audioTagNameToElement[tag];
		if(!modelElem) return;
		modelElem.pause();
	}
	this.fadeoutLongPlay = function(tag,timeInSeconds){
		var interval = timeInSeconds * (1000 / 10); //microseconds in ten steps
		this.turnDownForWhat(tag, interval, 1);
	}
	this.turnDownForWhat = function(tag,interval,volume){
		lowLag.msg("turnDownForWhat "+volume);
		var modelElem = lowLag.audioTagNameToElement[tag];
		volume -= .1;
		if(volume < 0) volume = 0;
		modelElem.volume = volume;
		if(volume <= 0) modelElem.pause();
		else setTimeout(function(){
			lowLag.turnDownForWhat(tag,interval,volume);
		},interval);
